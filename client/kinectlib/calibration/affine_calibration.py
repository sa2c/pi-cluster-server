...
def affine_transform_contour_dtc(contour):
    """
    This is a bit of a hack, but avoids changes to the rest of the code.
    Sometimes contour is passed in with shape [N, 1, 2], and is required
    back in the same shape. This code first transforms it is then
    tranformed back.
    """
    shape = contour.shape

    # tranform the shape if necessary
    if shape[0] > shape[1] and len(shape) == 3:
        transformed = True
        contour = contour[:, 0, :]
    ...
    contour = _affine_transform_contour_dtc(contour)

    # and tranform back again
...
def _affine_transform_contour_dtc(contour):
    ...
    ret = np.matmul(_calibration_dtc_M, contour3)[0:2, :]
